...
> One local binary. Open a browser. Discover and inspect gRPC or send HTTP
> requests without moving credentials or request history to a hosted service.
...
### 3. Technical communities: after package smoke tests

- Open a focused contribution to
...
- Submit a Show HN only when the stable build is runnable without signup. The
...
- **Proof:** no account, local history, secret-safe exports, bounded discovery,
  verified installers, and native gRPC/HTTP response views.
- **Boundary:** Cap'n Proto, route trace, and LAN discovery are roadmap work,
